...
var range = function(x, y) {
  ...
  if (y > x){
    return y - x === 2 ? [x + 1] : result.concat(x + 1, range(x + 1, y))
  } else {
    return x - y === 2 ? [x - 1] : result.concat(x - 1, range(x - 1, y));
  }
};
...
var exponent = function(base, exp) {
  ...
  if (exp < 0) {
    var result = 1 / exponent(base, exp * -1);
    return result;
  }
  if (exp % 2 === 0){
...
var countValuesInObj = function(obj, value) {
  ...
  if (typeof obj[Object.keys(obj)[0]] !== 'object'){
    return obj[Object.keys(obj)[0]] === value ? 1 : 0;
  }
